...
### Docs
- **README clarifies that `claude setup-token` requires an active OAuth session (#173).** Setup step 1 now splits into `claude login` (device-code flow on headless VMs) followed by `claude setup-token` — running `setup-token` alone produces an invalid bearer token that 401s on the first real API call. Caught during the same exploration test.
