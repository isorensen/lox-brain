...
O [[cicd-github-actions]] roda `npm run test:coverage` em todo PR para `main`. O threshold de 80% e enforced -- PR que baixa cobertura nao passa no CI.

Adicionalmente, `tsc --noEmit` e `npm audit` rodam no mesmo pipeline.

## Relacoes

- valida: [[embedding-service]], [[vault-watcher]], [[mcp-server]]
- integrado com: [[cicd-github-actions]]
- contido em: [[_MOC]]
