...
step3_activate_wg() {
  ...
  # Use ifconfig on macOS (no 'ip' command), ip on Linux.
  local iface_up=false
  if command -v ip &>/dev/null; then
    ip link show "$WG_IFACE" &>/dev/null 2>&1 && iface_up=true
  elif ifconfig "$WG_IFACE" &>/dev/null 2>&1; then
    iface_up=true
  fi
...
step7_verify() {
  ...
  echo ""
  echo "  If something went wrong, you can open an issue automatically:"
  echo "    ./scripts/team-onboard.sh --report-issue"
  echo ""
}
...
report_issue() {
  ...
  local diag=""
  diag+="## Environment\n"
  diag+="- OS: $(uname -s) $(uname -r)\n"
  diag+="- Shell: ${SHELL}\n"
  diag+="- Node: $(node --version 2>/dev/null || echo 'not found')\n"
  diag+="- WireGuard: $(wg --version 2>/dev/null || echo 'not found')\n"
  diag+="- Claude Code: $(claude --version 2>/dev/null || echo 'not found')\n"
  diag+="\n## WireGuard\n"

  if command -v ip &>/dev/null; then
    diag+="- Interface wg1: $(ip link show wg1 2>/dev/null && echo 'UP' || echo 'DOWN')\n"
  elif ifconfig wg1 &>/dev/null 2>&1; then
    diag+="- Interface wg1: UP\n"
  else
    diag+="- Interface wg1: DOWN\n"
  fi
  diag+="- Ping 10.20.0.1: $(ping -c 1 -W 3 10.20.0.1 &>/dev/null && echo 'OK' || echo 'FAIL')\n"

  diag+="\n## MCP\n"
  local mcp_status
  mcp_status="$(claude mcp list 2>/dev/null | grep "${MCP_NAME}" || echo 'not registered')"
  diag+="- ${MCP_NAME}: ${mcp_status}\n"

  diag+="\n## Config\n"
  diag+="- ~/.lox/config.json: $(test -f "${LOX_CONFIG}" && echo 'exists' || echo 'missing')\n"
  diag+="- WG config: $(test -f "${WG_TARGET_DIR}/${WG_IFACE}.conf" && echo 'exists' || echo 'missing')\n"

  local skills_count
  skills_count="$(find "${SKILLS_TARGET}" -name "SKILL.md" 2>/dev/null | wc -l | tr -d ' ')"
  diag+="- Skills installed: ${skills_count}\n"

  echo -e "$diag"

  if ! command -v gh &>/dev/null; then
    warn "'gh' (GitHub CLI) not installed — cannot create issue automatically."
    echo "  Copy the diagnostics above and open an issue at:"
    echo "  https://github.com/isorensen/lox-brain/issues/new"
    return
  fi

  printf "Open a GitHub issue with these diagnostics? [y/N] "
  read -r answer
  if [[ ! "$answer" =~ ^[Yy]$ ]]; then
    info "Skipped. You can paste the diagnostics into a manual issue."
    return
  fi

  local body
  body="$(echo -e "## Onboarding issue\n\nAutomatically generated by \`team-onboard.sh --report-issue\`.\n\n${diag}")"

  gh issue create \
    --repo isorensen/lox-brain \
    --title "team onboarding: setup failed for $(whoami)@$(hostname)" \
    --body "$body" \
    --label "bug" 2>&1
