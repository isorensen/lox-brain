...
describe('PeerResolver', () => {
  const peers: VpnPeer[] = [
    ...
    { name: 'matheus', ip: '10.20.0.3', public_key: 'key2', added_at: '2026-04-03' },
    { name: 'igor', ip: '10.20.0.4', public_key: 'key3', added_at: '2026-04-03' },
  ];

  it('should resolve a known IP to its peer identity', () => {
    const resolver = new PeerResolver(peers);
    const result = resolver.resolve('10.20.0.2');
  ...
  it('should return null for an unknown IP', () => {
    const resolver = new PeerResolver(peers);
    expect(resolver.resolve('10.20.0.99')).toBeNull();
  });

  it('should return null for the server IP', () => {
    const resolver = new PeerResolver(peers);
    expect(resolver.resolve('10.20.0.1')).toBeNull();
  });
  ...
  it('should handle IPv6-mapped IPv4 addresses', () => {
    const resolver = new PeerResolver(peers);
    const result = resolver.resolve('::ffff:10.20.0.3');
    expect(result).not.toBeNull();
  ...
  it('should handle empty peer list', () => {
    ...
    expect(resolver.resolve('10.20.0.2')).toBeNull();
  });
